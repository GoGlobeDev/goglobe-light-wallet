...
class decomposePower extends React.Component {
	static navigationOptions = ({navigation}) => ({
        headerTitle: <View style={{ alignItems: 'center', flexDirection: 'row', justifyContent: 'center', width: scaleSize(523), paddingLeft: 0 }}>
          <Text style={{ fontSize: scaleSize(34), color: 'rgba(50, 50, 50, 1)', marginLeft: 0 }}>{I18n.t('node.powers._title')}</Text>
        </View>,
        headerRight: <TouchableOpacity onPress={() => navigation.state.params.currencyRule()}>
          <Text style={{ paddingRight: scaleSize(33), fontSize: 16, color: '#486495' }}>{I18n.t('node.powers.powerRule')}</Text>
        </TouchableOpacity>,
      })
	constructor(props) {
		super(props);
	...
	componentDidMount() {
        ...
        this.props.navigation.setParams({
            currencyRule: () => {
                this.props.navigation.navigate('powerRule')
            }
        })
    }
    clickToAllWithdraw = () => {
		...
		return (
			<View style={styles.container}>
				<View style={styles.shadowView}>
                    <Text style={styles.text14}>{I18n.t('node.powers.powerAmount')}{I18n.t('node.powers.powerFee')}</Text>
                    ...
                    <View style={[styles.rows, { justifyContent: 'space-between', marginTop: scaleSize(25)}]}>
                        <Text style={[styles.text14, { opacity: 0.5 }]}>{I18n.t('node.powers.availableBalance')}：{show(this.state.gog_banlance)}GOG</Text>
                        <TouchableOpacity onPress={() => this.clickToAllWithdraw()}>
                            <Text style={[styles.text14, { color: '#486495'} ]}>{I18n.t('node.powers.powerAll')}</Text>
                        </TouchableOpacity>
                    </View>
                    <Text style={styles.text17}>{I18n.t('node.powers.receivedAmount')}：{show(this.state.banlance)}</Text>
                    <TouchableOpacity style={[styles.button, this.state.number === '' ? { backgroundColor: '#F7C9A9' } : {  backgroundColor: '#EA7828' }]} onPress={this._clickTocomfirm}>
                        <Text style={{color: 'rgba(255,255,255,1)', fontSize: 17, textAlign: 'center'}}>{I18n.t('node.powers.powerToken')}</Text>
                    </TouchableOpacity>
                </View>
                <Loading ref="loading" />
                <Modal
